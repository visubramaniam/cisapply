...
def generate_markdown_report(report: Dict[str, Any], md_path: str) -> bool:
    ...
    try:
        ...
        status_emoji = "\u2705" if report.get("ok") else "\u274c"
        
        md_content = f"""# CIS Compliance Report - {report.get('profile', 'unknown').upper()}

## Summary

| Metric | Value |
|--------|-------|
| **Profile** | {report.get('profile', 'unknown').upper()} |
| **Hostname** | {metadata.get('hostname', 'N/A')} |
| **OS** | {metadata.get('os_release', 'N/A')} |
| **Kernel** | {metadata.get('kernel', 'N/A')} |
| **Benchmark** | {metadata.get('cis_benchmark', 'N/A')} |
...
| **Overall Status** | {status_emoji} {'PASS' if report.get('ok') else 'FAIL'} |

## Remediation Summary

| Action | Count |
|--------|-------|
| Remediated | {remediation.get('remediated', 0)} |
| Already Compliant | {remediation.get('already_compliant', 0)} |
...
"""
        ...
        for r in report.get("results", []):
            status_icon = "\u2705" if r.get("ok") else "\u274c"
            changed_text = "Yes" if r.get("changed") else "No"
            control_id = r.get("id", "N/A")
            cis_ref = r.get("cis_control", CONTROL_MAPPING.get(control_id, ""))
            if not cis_ref:
                # Try matching base ID (e.g., LOG-6 from LOG-6-/var/log/wtmp)
                base_id = control_id.split('-')[0] + '-' + control_id.split('-')[1] if '-' in control_id and len(control_id.split('-')) > 1 else control_id
                cis_ref = CONTROL_MAPPING.get(base_id, "N/A")
            title = r.get("title", "N/A")
            notes = r.get("notes", "").replace("|", "/").replace("\n", " ")[:60]
            if len(r.get("notes", "")) > 60:
                notes += "..."
            
            md_content += f"| {status_icon} | `{control_id}` | {cis_ref} | {title} | {changed_text} | {notes} |\n"
...
def main():
    ...
    # Save Markdown report - auto-generate for --apply mode
    if args.apply:
        generate_markdown_report(report, "hardening_report.md")
